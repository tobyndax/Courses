...
to $0.7*10^{-3}$. This one is also close to zero which ones again indicates a
pure whistle.
...
the systems are compared with bode plots of the data. The a-sound presented in
...
Each pulse train is as long as a segment, 160 samples.
...
we draw the conclusion that orders lower than $8$ give unsatisfactory
...
except for the one set to one, period time and segment amplitude.
